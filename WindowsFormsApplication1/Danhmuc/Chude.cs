...
namespace WindowsFormsApplication1
{
    public partial class Chude : Form
    {
        ...
        {
            dataGridView1.DataSource = taikhoan.Loadchude();
            // wrap text
            ...
            dataGridView1.Columns[1].HeaderText = "Tên chủ đề";
        ...
        {
            bool tontai = false;
            foreach (bantin_chude item in taikhoan.Loadchude())
            {
                if (item.tenchude == textBox1.Text)
                    tontai = true;
            }
            if (tontai)
            ...
            {
                taikhoan.Themchude(textBox1.Text);
                MessageBox.Show("Thêm chủ đề mới thành công", "Thông báo");
                loaddata();
            }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dialogResult = MessageBox.Show(string.Format("Cập nhật chủ đề {0} thành {1}?", tencu, tenmoi),
                "Thông báo", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                taikhoan.Capnhatchude(tencu, tenmoi);
            }
            loaddata();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DialogResult dialogResult = MessageBox.Show(string.Format("Xóa chủ đề {0}?", tenxoa),
                "Thông báo", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                {
                    taikhoan.Xoachude(tenxoa);
            ...
            catch
            {
                MessageBox.Show(@"Không thể xóa
                                  Tồn tại bản tin có chủ đề này", "Lỗi");
            }
            finally
            {
                loaddata();
